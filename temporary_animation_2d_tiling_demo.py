...
#ANIMATION CODE#
'''
There are still clearly some issues to address! I'll try adapt this to new code and improve this tomorrow!
'''



def rotate_transformation(tiling3, i, theta_x = 10.0, theta_y = 10.0):
...
def animate_tiling(tiling3 = cubic_tiling3(-3,3,-3,3,-3,3).translate(Vector3(0,0,-0.51)),\
    ...
    xticks = range(-10,11), yticks= range(-10,11) , aspect='equal', frameon = True)
    axis.grid(True)
    kwds = dict(ec='k', alpha = 0.75)
    #transforming_3d_tiling = tiling3
    ...
    def animate(i):
        ...
        patches = []
        index = 0
        for face in transforming_2d_intersection.faces.keys():
            patches += [axis.add_patch(plt.Polygon(0 * np.array(prepare_tiling2_face(face)), facecolor =\
            colours[transforming_2d_intersection.faces.keys().index(face)%len(colours)], **kwds))]    
            patches[index].set_xy(np.array(prepare_tiling2_face(face)))
            index += 1
        return patches
...
animate_tiling(transformation_function = rotate_transformation)    
